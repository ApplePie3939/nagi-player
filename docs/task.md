...
- 動画のインライン再生、全画面表示、および対応ブラウザでの Picture in Picture（PiP）
...
- 再生リスト選択、再生／一時停止、シーク、前後移動、再生終了後の次項目遷移が動作する。再生中に前後移動した場合は、移動先を続けて再生する。
- 音声と動画を続けて切り替えても同時再生しない。
- 対応ブラウザではMedia Session、動画の全画面表示およびPiPを利用でき、非対応環境でも通常再生ができる。
...
バックグラウンド再生、ロック画面操作、Media Session、動画の全画面表示、PiPの可否や動作範囲は、ブラウザとOSの対応状況に依存する。これらはネイティブアプリと同等の動作を保証しない。
